			...
			if (args.getArgs().length == 0)
				...
				switch (args.getArgs()[0]) {
					case "set": {
						...
						final List<Block> blocks = Cuboid.blocksFromTwoPointsEx(WorldModifierHandler.getPos1().get(player), WorldModifierHandler.getPos2().get(player), mat);
						...
						player.sendMessage(Strings.TITLE + "Setting " + blocks.size() + " blocks to " + TextUtils.capitalize(mat.name().toLowerCase().replace('_', ' ')) + " (Estimate " + ((blocks.size() / divisor) / 4) + " seconds)");
