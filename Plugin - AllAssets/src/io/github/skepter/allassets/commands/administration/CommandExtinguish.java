	...
	public void command(final CommandArgs args) {
		...
		if (player != null) {
			switch (args.getArgs().length) {
				case 0:
					final Sphere sphere = new Sphere(player.getLocation(), 120, Material.FIRE);
					for (final Block b : sphere.getBlocks())
						b.setType(Material.AIR);
					player.sendMessage(Strings.TITLE + sphere.getBlocks().size() + " fires removed");
					return;
				case 1:
					if (TextUtils.isInteger(args.getArgs()[0])) {
						final Sphere sphereWithCustomRadius = new Sphere(player.getLocation(), Integer.parseInt(args.getArgs()[0]), Material.FIRE);
						for (final Block b : sphereWithCustomRadius.getBlocks())
							b.setType(Material.AIR);
