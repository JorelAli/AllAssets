...
/**
 * Makes creating columns a LOT easier.
 * All SQL constraints are not included by default.
 */
public class ColumnBuilder {
    private String name = "a_column", dataType = "VARCHAR(225)", def = null, foreignKey = null;
     ...
     * @param unique {@code true} if this column should ensure that every row has a unique value
     * @return {@code this} for chaining
     */
     ...
     * @param primaryKey {@code true} if this column is the primary key for the table
     * @return {@code this} for chaining
     */
     ...
     * @param def the default value for rows in this column.
     *            You can use functions, values may need to be surrounded by ''.
     * @return {@code this} for chaining
     */
    public ColumnBuilder setDefault(String def) {
        this.def = def;
        return this;
    }
     ...
     * @param foreignKey the column to correspond with in the format {@code TABLE(COLUMN)}
     * @return {@code this} for chaining
     */
    public ColumnBuilder setForeignKey(String foreignKey) {
    ...
    /**
     * Gets the foreign key string for this column.
     * It is advised that you check if the foreign key exists using {@link ColumnBuilder#foreignKey}
